...
async def app_status(request: Request) -> JSONResponse:
    ...
    checks["accounting"] = get_accounting_provider().configured()
...
def _build_payfast_url(first: str, last: str, email: str, amount_cents: int, item_name: str, booking_id: int) -> str:
    ...
    params = {
        ...
        "notify_url": os.environ.get("SMARTBIZ_API_URL", "http://localhost:8000") + "/payfast/notify",
...
async def get_technician_qr(request: Request) -> JSONResponse:
    ...
    qr_url = (os.environ.get("SMARTBIZ_API_URL", "http://localhost:8000") + f"/technician/complete/{booking_id}?token={complete_token}").replace(" ", "")
    try:
...
async def create_booking_inspection(request: Request) -> JSONResponse:
    booking_id = int(request.path_params["booking_id"])
    ...
    with lock, sqlite3.connect(DB_PATH) as con:
        ...
        if existing:
            return JSONResponse({
                "ok": True,
                "inspection": dict(existing),
                "existing": True,
            })

        technician_id = payload.get(
            "technician_id",
            booking["assigned_technician_id"] or None,
        )
...
async def update_inspection(request: Request) -> JSONResponse:
    ...
    if not updates:
        return _json_error("no inspection fields supplied", 400)
